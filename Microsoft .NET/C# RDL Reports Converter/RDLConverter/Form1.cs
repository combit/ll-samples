...
using combit.ListLabel24.Converters;
...
namespace WindowsFormsApplication1
{
    public partial class Form1 : MetroForm
    {
        ...
        public Form1()
        {
            ...
            if (installKey != null)
                sampleDir = (string)installKey.GetValue("LL24SampleDir", string.Empty);

            if (!sampleDir.EndsWith("\\"))
                sampleDir += "\\";

            SourceFileName = sampleDir + @"Microsoft .NET\C# Rdl Reports Converter\RdlConverter\RDlReports.rdl";
            TargetFileName = sampleDir + @"Microsoft .NET\C# Rdl Reports Converter\RdlConverter\ReportConvert.lst";
        ...
        {
            if ((sender as MetroFramework.Controls.MetroTextBox).Name == textSourceFile.Name)
                toolTip1.Show(textSourceFile.Text, textSourceFile);
            else if ((sender as MetroFramework.Controls.MetroTextBox).Name == textTargetFile.Name)
                toolTip1.Show(textTargetFile.Text, textTargetFile);
        }

        private void TextSourceFile_MouseLeave(object sender, EventArgs e)
        {
            toolTip1.Hide((sender as MetroFramework.Controls.MetroTextBox));
        }
    }
}
