...
namespace TXTextDesignerObject
{
    ...
    public class TXDesignerObject : DesignerObject
    {
        ...
        public TXDesignerObject(String objectName, String objectDescription, String objectIconFile, ListLabel parentListLabel)
        {
            ...
            ObjectName = String.IsNullOrEmpty(objectName) ? "TXTextObjectName" : objectName;

            // Set the description of the designer object in toolbar/ribbon
            Description = String.IsNullOrEmpty(objectDescription) ? "TX Text Control" : objectDescription;

            // Set the icon of the designer object (old/classic symbolbar)
            Icon = String.IsNullOrEmpty(objectIconFile) || !File.Exists(objectIconFile)
                ? TX_Text_Control_Designer_Object.Properties.Resources.TxText
                : new Icon(objectIconFile);
        ...
        {
            using EditDesignerObjectEventArgs editArgs = new(e.DesignerWindow.Handle);
        ...
        protected override void OnGetFieldHeightInformation(GetFieldHeightInformationEventArgs e)
        {
            ...
            PageSize newPageSize = new()
            {
                Height = ConvertLLUnitsToTXUnits(UnitSystem, usedHeight) + 1,
                Width = ConvertLLUnitsToTXUnits(UnitSystem, e.AvailableSpace.Width) + 1
            };
        ...
        protected override void OnDrawDesignerObject(DrawDesignerObjectEventArgs e)
        {
            ...
            if (IsInTableCell)
            {
                if (_calculatedHeightInTable > e.ClipRectangle.Height || _rtfPageNumber == 0 || e.IsDesignMode)
                {
                    ...
                    PageSize newPageSize = new()
                    {
                        Height = ConvertLLUnitsToTXUnits(UnitSystem, e.ClipRectangle.Height) + 1,
                        Width = ConvertLLUnitsToTXUnits(UnitSystem, e.ClipRectangle.Width) + 1
                    };
            ...
            Rectangle lobjRectangle = new(e.ClipRectangle.Left, e.ClipRectangle.Top, Convert.ToInt32(CoordinateHelper.ConvertUnit(Convert.ToDouble(_serverTXTextControl.GetPages()[1].TextBounds.Width), CoordinateHelper.ConversionUnit.Twip, UnitSystem, null)), Convert.ToInt32(CoordinateHelper.ConvertUnit(Convert.ToDouble(_serverTXTextControl.GetPages()[1].TextBounds.Height), CoordinateHelper.ConversionUnit.Twip, UnitSystem, null)));
        ...
        {
            List<string> lstIdentifiers = new();
            CollectRTFIdentifiers(ref lstIdentifiers);
            string currentRTFDocumentContent = DataSourceType == DataSource.Identifier ? Formula : DocumentRTFContent;

            using FormEditTXTextControl editTXTextControlDlg = new(lstIdentifiers, DataSourceType, currentRTFDocumentContent, Parent);
            NativeWindow nativeWindows = new();
            nativeWindows.AssignHandle(e.DesignerWindow.Handle);
            DialogResult dialogResult = editTXTextControlDlg.ShowDialog(nativeWindows);
        ...
        private bool IsPrinting
        {
            get
            {
                return ((int)Parent.Core.LlGetOption(337 /* LL_OPTION_IS_PRINTING */) == 1);
            }
        }
        ...
        private bool SuppressLoadErrorMessages
        {
            get
            {
                return ((int)Parent.Core.LlGetOption(LlOption.SuppressLoadErrorMessages /* 223 */) == 1);
            }
        }
        ...
        private static bool RTFContentContainsLLFormulas(ref string sContents, bool bReplace = true)
        {
