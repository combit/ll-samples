...
namespace Repository
{
    ...
    public class SQLiteFileRepository : IRepository
    {
        ...
        {
            databasePath = Path.Combine(Application.StartupPath + @"\..\..\..\App_Data\repository.db");
        ...
        {
            // Uncomment this to prevent the printer config file ("P-file") from being stored in the repository
            //if (item.Type == RepositoryItemType.PrinterConfig.Value)
            //    return;

            // Uncomment this to prevent the sketch image file ("V-file") from being stored in the repository
            //if (item.Type == RepositoryItemType.SketchImage.Value)
            //    return;

            // Set the user name to use for the Author column of the RepositoryItem
            string currentUser = "combit";
        ...
        private IEnumerable<CustomizedRepositoryItem> GetItemsFromDb(string itemId = null)
        {
            ...
            var cmd = _db.CreateCommand($"SELECT ItemID, Type, Descriptor, TimestampUTC, Author, ShowInToolbar, OriginalFileName, LENGTH(FileContent), Language FROM RepoItems WHERE (Language=@Language or Language is null)");
