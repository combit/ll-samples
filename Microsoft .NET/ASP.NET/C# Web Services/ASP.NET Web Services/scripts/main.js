...
function DataSourceSelected(anker)
{
    if (anker && anker.id)
    {
        ...
        combit.Services.ReportingService.GetReportTemplates(datasourceID, OnGetReportTemplates, OnError, null);
...
function OnGetDataSources(response, eventArgs)
{
    ...
    {                
        ...
        combit.Services.ReportingService.GetReportTemplates(response.Value[0].Id, OnGetReportTemplates, OnError, null);
    }
...
{
    combit.Services.ReportingService.Print(datasource, report, format, OnPrint, OnError, CreateResultWindow(report));

    // D: Nach dem Druck wieder zu den Berichten wechseln
    // US: Display reports after print
    combit.Services.ReportingService.GetReportTemplates(datasource, OnGetReportTemplates, OnError, null);
}
...
function GetAvailableFormats(report)
{
    ...
    combit.Services.ReportingService.GetAvailableFormats(OnGetFormats, OnError, null);
}
...
{    
    ...
    combit.Services.ReportingService.GetDataSources(OnGetDataSources, OnError, null);
