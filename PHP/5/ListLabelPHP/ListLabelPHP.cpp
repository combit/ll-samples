#include "stdafx.h"
#define _NO_CMLL27APIDEFINES
#include "..\..\Visual C++\cmbtll27.h"
...
void CheckErrorCode(int nError)
{
	...
	{ 
		...
		MAKE_STD_ZVAL(oBuffer);
		oBuffer->type = IS_STRING;
		oBuffer->value.str.len = strlen(aczContents);
		oBuffer->value.str.val = estrdup((const char*)aczContents);
		delete[] aczContents;
		char* aczBuffer;
		aczBuffer = new char[strlen((const char*)aczContents) + 10];
		sprintf((char*)aczBuffer, (const char*)"%s (%d)", oBuffer->value.str.val, nError);
...
ZEND_FUNCTION(LlDbAddTable) 
{
	...
	int pszTableID_len;
	int pszDisplayName_len;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "lss", &hJob, &pszTableID, &pszTableID_len, &pszDisplayName, &pszDisplayName_len) != FAILURE)
	{
        int nRet = LlDbAddTableA(hJob, (LPCSTR)pszTableID, (LPCSTR)pszDisplayName);
		CheckErrorCode(nRet);
    }
}
// LlDebugOutput
...
{	
	...
	LPCTSTR pszText;
	int str_len;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ls", &nIdent, &pszText, &str_len) != FAILURE)
	{
        ::LlDebugOutputA(nIdent, (LPCSTR)pszText);
    }
}
// LlDefineField
ZEND_FUNCTION(LlDefineField)
{
	...
	int lpszCont_len;
	long nRet = 0;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "lss", &hJob, &lpszName, &lpszName_len, &lpszCont, &lpszCont_len) != FAILURE)
	{
        nRet = ::LlDefineFieldA(hJob, lpszName, lpszCont);
		CheckErrorCode(nRet);
    }
}
// LlDefineFieldExt
ZEND_FUNCTION(LlDefineFieldExt)
{
	...
	LPVOID lpPara = NULL;
	zval * zvPara;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "lsslz", &hJob, &lpszName, &lpszName_len, &lpszCont, &lpszCont_len, &lPara, &zvPara) != FAILURE)
	{
        nRet = ::LlDefineFieldExtA(hJob, lpszName, lpszCont, lPara, NULL);
		CheckErrorCode(nRet);
    }	
}
// LlDefineVariable
ZEND_FUNCTION(LlDefineVariable)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "lss", &hJob, &lpszName, &lpszName_len, &lpszCont, &lpszCont_len) != FAILURE)
	{
        nRet = ::LlDefineVariableA(hJob, lpszName, lpszCont);
	    CheckErrorCode(nRet);
    }
}
...
{
	long hJob;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "l", &hJob) != FAILURE)
	{
...
{
	long nError;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "l", &nError) != FAILURE)
	{
		zval* oBuffer;
		TCHAR* string_contents;
		int nErrTxteLen = ::LlGetErrortextA(nError, NULL, 0);
		string_contents = new TCHAR[nErrTxteLen];
		int nRet = ::LlGetErrortextA(nError, (LPSTR)string_contents, nErrTxteLen); 
		MAKE_STD_ZVAL(oBuffer);
		oBuffer->type = IS_STRING;
		oBuffer->value.str.len = _tcslen(string_contents);
		oBuffer->value.str.val = estrdup((const char *)string_contents);
		convert_to_string_ex(&oBuffer);
		delete[] string_contents;
		CheckErrorCode(nRet);
		RETURN_STRING(oBuffer->value.str.val, 0);
    }
}
...
ZEND_FUNCTION(LlGetVariableContents)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ls", &hJob, &lpszName, &lpszName_len) != FAILURE)
	{
        zval* oBuffer;
		TCHAR* string_contents;
		string_contents = new TCHAR[PHP_MAXSTRLEN];
		long nRet = ::LlGetVariableContentsA(hJob, lpszName, (LPSTR)string_contents, PHP_MAXSTRLEN);
		MAKE_STD_ZVAL(oBuffer);
		oBuffer->type = IS_STRING;
		oBuffer->value.str.len = _tcslen(string_contents);
		oBuffer->value.str.val = estrdup((const char *)string_contents);
...
{
	int nCmd;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "l", &nCmd) != FAILURE)
	{
        RETURN_LONG(::LlGetVersion(nCmd));
    }
}
// LlJobClose
ZEND_FUNCTION(LlJobClose)
...
ZEND_FUNCTION(LlPrintEnd)
{
	long hJob;
	long nPages;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ll", &hJob, &nPages) != FAILURE)
	{
        int nRet = ::LlPrintEnd(hJob, nPages);
		CheckErrorCode(nRet);
    }
}
// LlPrintFields
...
ZEND_FUNCTION(LlPrintFieldsEnd)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "l", &hJob)!= FAILURE)
	{
        int nRet = ::LlPrintFieldsEnd(hJob);
		RETURN_LONG(nRet);
    }
}
...
ZEND_FUNCTION(LlPrintGetCurrentPage)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "l", &hJob) != FAILURE)
	{
        int nRet = ::LlPrintGetCurrentPage(hJob);
		RETURN_LONG(nRet);
    }
}
// LlPrintGetOption
ZEND_FUNCTION(LlPrintGetOption)
{
	long hJob;
	long nIndex;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ll", &hJob, &nIndex) != FAILURE)
	{
        int nRet = ::LlPrintGetOption(hJob, nIndex);
		RETURN_LONG(nRet);
    }
}
...
ZEND_FUNCTION(LlPrintGetOptionString)
{
	long hJob;
	long nMode;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "llz", &hJob, &nMode) != FAILURE)
	{
        zval* oBuffer;
		TCHAR* string_contents;
		string_contents = new TCHAR[PHP_MAXSTRLEN];
		long nRet = ::LlPrintGetOptionStringA(hJob, nMode, (LPSTR)string_contents, PHP_MAXSTRLEN);
		MAKE_STD_ZVAL(oBuffer);
		oBuffer->type = IS_STRING;
		oBuffer->value.str.len = _tcslen(string_contents);
		oBuffer->value.str.val = estrdup((const char *)string_contents);
...
ZEND_FUNCTION(LlPrintSetOptionString)
{
	...
	LPCTSTR pszValue;
	int pszValue_len;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "lls", &hJob, &nMode, &pszValue, &pszValue_len) != FAILURE)
	{
        int nRet = LlPrintSetOptionStringA(hJob, nMode, (LPCSTR)pszValue);
		CheckErrorCode(nRet);
    }
}
// LlPrintStart
ZEND_FUNCTION(LlPrintStart)
{
	...
	int nPrintOptions;
	int nV1Dummy;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "llsll", &hJob, &nObjType, &lpszObjName, &lpszObjName_len, &nPrintOptions, &nV1Dummy) != FAILURE)
	{
        int nRet = ::LlPrintStartA(hJob, nObjType, lpszObjName, nPrintOptions, nV1Dummy);
		CheckErrorCode(nRet);
    }
}
// LlSetDebug
...
{
	long nOnOff;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "l", &nOnOff) != FAILURE)
	{
        ::LlSetDebug(nOnOff);
    }	
}
...
ZEND_FUNCTION(LlXSetParameter)
{
	...
	int pszValue_len;
	int pszKey_len;
	int pszExtensionName_len;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "llsss", &hJob, &nExtensionType, &pszExtensionName, &pszExtensionName_len, &pszKey, &pszKey_len, &pszValue, &pszValue_len) != FAILURE){
		int nRet = LlXSetParameterA(hJob, nExtensionType, (LPCSTR)pszExtensionName, (LPCSTR)pszKey, (LPCSTR)pszValue);
		CheckErrorCode(nRet);
    }
}
...
ZEND_FUNCTION(LlSetOptionString)
{
	...
	int str_len;
	LPCSTR pszBuffer;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "lls", &hJob, &nIndex, &pszBuffer, &str_len) != FAILURE)
	{
		int nRet = ::LlSetOptionStringA(hJob, nIndex, pszBuffer);
		CheckErrorCode(nRet);
	}
}
